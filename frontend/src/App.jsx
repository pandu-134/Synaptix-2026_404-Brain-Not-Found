...
import { Brain, TrendingUp, Target, Award, Play, Activity, CheckCircle2, XCircle, CheckCircle, Flame, Sparkles, ChevronRight, Zap, AlertTriangle, BookOpen, ArrowLeft } from 'lucide-react';
...
// --- TEST TOPICS DATA (NEW) ---
const testTopics = [
  { id: 'ds', name: 'Data Structures', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'algo', name: 'Algorithms', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'os', name: 'Operating Systems', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'dbms', name: 'DBMS', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'cn', name: 'Computer Networks', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'se', name: 'Software Engineering', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'oop', name: 'OOP', qCount: TOTAL_QUESTIONS, time: 15 },
  { id: 'aptitude', name: 'Aptitude & Reasoning', qCount: TOTAL_QUESTIONS, time: 15 },
];
...
export default function App() {
  const [currentView, setCurrentView] = useState('dashboard'); // Added 'topics' to the flow
  const [selectedActiveTopic, setSelectedActiveTopic] = useState(""); // Tracks the chosen topic
  const [studentData, setStudentData] = useState({
    name: "Harish", level: "Advanced Scholar", totalTests: 14, avgAccuracy: 68, streak: 5,
    skills: [
      { name: "Data Structures", mastery: 45, color: "from-fuchsia-400 to-purple-600", shadow: "shadow-purple-500/50" },
      { name: "Algorithms", mastery: 65, color: "from-emerald-400 to-teal-600", shadow: "shadow-teal-500/50" },
      { name: "Operating Systems", mastery: 82, color: "from-cyan-400 to-blue-600", shadow: "shadow-cyan-500/50" },
      { name: "DBMS", mastery: 70, color: "from-amber-400 to-orange-600", shadow: "shadow-orange-500/50" },
      { name: "Computer Networks", mastery: 55, color: "from-rose-400 to-red-600", shadow: "shadow-red-500/50" },
      { name: "Software Engineering", mastery: 88, color: "from-indigo-400 to-violet-600", shadow: "shadow-indigo-500/50" },
      { name: "OOP", mastery: 60, color: "from-lime-400 to-green-600", shadow: "shadow-green-500/50" },
      { name: "Aptitude & Reasoning", mastery: 75, color: "from-sky-400 to-blue-600", shadow: "shadow-sky-500/50" },
    ]
  });
  ...
  const startTest = async (topicName) => {
    setSelectedActiveTopic(topicName); // Save what they clicked!
    setIsLoading(true);
    setCurrentView('test');
    setSessionScore(0);
    ...
    try {
      // Send the topic they clicked to the backend!
      const response = await fetch(`http://127.0.0.1:5001/api/start-test?topic=${encodeURIComponent(topicName)}`);
  ...
  const handleNextQuestion = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5001/api/submit-answer', {
        ...
        body: JSON.stringify({
          topic: selectedActiveTopic, // Using the topic they chose from the grid
  ...
  const calculateNewStats = (finalScore) => {
    ...
    setStudentData(prev => {
      ...
      const updatedSkills = prev.skills.map(skill => {
        let masteryChange = (finalScore / TOTAL_QUESTIONS) >= 0.5 ? 25 : -10; 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="group relative p-[1px] rounded-[2rem] overflow-hidden bg-gradient-to-b from-fuchsia-500 to-cyan-500 shadow-[0_0_40px_rgba(217,70,239,0.3)] hover:shadow-[0_0_60px_rgba(217,70,239,0.5)] transition-all duration-500">
              ...
              <div className="relative h-full bg-[#0F1423] p-8 rounded-[2rem] flex flex-col justify-center items-center text-center">
                ...
                <button onClick={openTopicSelection} className="w-full py-5 bg-white text-[#0B0F19] font-black text-lg rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:scale-105 transition-all duration-300 flex justify-center items-center group/btn"><Play className="mr-2 group-hover/btn:text-fuchsia-500 transition-colors" fill="currentColor" size={22}/> Choose Topic</button>
              </div>
            </motion.div>
          </div>
  ...
  // --- VIEW 2: TOPIC SELECTION (NEW GRID) ---
  if (currentView === 'topics') {
    return (
      <div className="min-h-screen bg-[#0B0F19] text-slate-100 p-6 md:p-12 font-sans overflow-x-hidden">
        <div className="fixed top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-cyan-600/10 blur-[120px] rounded-full pointer-events-none"></div>
        
        <div className="max-w-6xl mx-auto relative z-10">
          <button onClick={() => setCurrentView('dashboard')} className="mb-8 flex items-center text-slate-400 hover:text-white transition-colors font-semibold">
            <ArrowLeft className="mr-2" size={20}/> Back to Dashboard
          </button>

          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
            <h1 className="text-4xl md:text-5xl font-black text-white mb-4 flex items-center tracking-tight">
              <BookOpen className="mr-4 text-cyan-400" size={40}/> Start a Test
            </h1>
            <p className="text-slate-400 text-lg font-medium">Choose a subject to begin your adaptive assessment. Each test has {TOTAL_QUESTIONS} questions.</p>
          </motion.div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {testTopics.map((topic) => (
              <motion.div 
                ...
                onClick={() => startTest(topic.name)}
                className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 hover:border-cyan-400/50 p-6 md:p-8 rounded-3xl cursor-pointer group transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(6,182,212,0.15)]"
              >
                <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-cyan-400 transition-colors">{topic.name}</h3>
                <p className="text-slate-400 mb-6 font-medium">{topic.qCount} questions • {topic.time} min limit</p>
                
                <div className="flex gap-3">
                  <span className="bg-slate-950/80 text-slate-300 text-sm font-semibold px-4 py-1.5 rounded-full border border-slate-700/50 group-hover:border-emerald-500/30 transition-colors">Easy 4</span>
                  <span className="bg-slate-950/80 text-slate-300 text-sm font-semibold px-4 py-1.5 rounded-full border border-slate-700/50 group-hover:border-amber-500/30 transition-colors">Med 4</span>
                  <span className="bg-slate-950/80 text-slate-300 text-sm font-semibold px-4 py-1.5 rounded-full border border-slate-700/50 group-hover:border-rose-500/30 transition-colors">Hard 2</span>
                </div>
              </motion.div>
            ))}
          </motion.div>
    ...
    if (isLoading || !currentQuestion) {
      return (
        <div className="min-h-screen bg-[#0B0F19] flex flex-col items-center justify-center p-6 font-sans">
          <div className="fixed top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-cyan-600/20 blur-[120px] rounded-full pointer-events-none"></div>
          <Activity className="animate-spin text-cyan-400 mb-8" size={64} />
          <h2 className="text-3xl font-black text-white mb-2 tracking-tight">Synthesizing Neural Node...</h2>
          <p className="text-slate-400 text-lg font-medium">AI is generating a custom {selectedActiveTopic} question.</p>
        </div>
      );
    }
    ...
    return (
      <div className="min-h-screen bg-[#0B0F19] flex flex-col items-center justify-center p-6 font-sans selection:bg-cyan-500/30">
        ...
        <div className="w-full max-w-3xl flex justify-between mb-8 text-sm font-bold tracking-widest uppercase">
          {/* Displays the topic they picked from the grid! */}
          <span className="text-cyan-400 flex items-center bg-cyan-950/30 px-4 py-2 rounded-xl border border-cyan-900/50"><Target className="mr-2" size={16}/> {selectedActiveTopic}</span>
        ...
        <div className="w-full max-w-3xl relative">
          <AnimatePresence mode="wait">
            ...
            >
              <h2 className="text-2xl md:text-4xl font-semibold mb-10 text-white leading-tight tracking-tight whitespace-pre-wrap">{formatAIText(currentQuestion.text)}</h2>
              <div className="space-y-4">
                {Object.entries(currentQuestion.options).map(([key, value]) => (
                  ...
                  >
                    ...
                    <span className={`text-xl font-medium tracking-wide whitespace-pre-wrap ${selectedOption === key ? 'text-cyan-50' : 'text-slate-300'}`}>{formatAIText(value)}</span>
                  </button>
                ))}
              </div>
              <div className="mt-12 flex justify-end">
                ...
                >
                  {questionsAnswered === TOTAL_QUESTIONS - 1 ? 'Execute Evaluation' : 'Lock Answer & Proceed'} <ChevronRight className="ml-2"/>
                </button>
              </div>
            </motion.div>
          </AnimatePresence>
      ...
      <div className="min-h-screen bg-[#0B0F19] flex flex-col items-center py-16 px-6 font-sans text-white relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-4xl relative z-10">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-slate-400 font-medium">Topic: <span className="text-cyan-400 font-bold">{selectedActiveTopic}</span></p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
             <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className={`p-[1px] rounded-[2.5rem] bg-gradient-to-b ${isSuccess ? 'from-emerald-400 to-teal-600 shadow-[0_0_40px_rgba(16,185,129,0.2)]' : 'from-red-400 to-rose-600 shadow-[0_0_40px_rgba(244,63,94,0.2)]'}`}>
                <div className="bg-[#0B0F19]/90 backdrop-blur-xl h-full p-10 rounded-[2.5rem] flex flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-4 text-slate-300 font-bold text-lg bg-slate-800/50 px-6 py-2 rounded-full">{sessionScore} / {TOTAL_QUESTIONS} Data Points Verified</p>
                </div>
             </motion.div>
             ...
             <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="bg-slate-900/50 backdrop-blur-xl p-10 rounded-[2.5rem] border border-slate-700/50 shadow-xl space-y-6 flex flex-col justify-center">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center"><span className="text-slate-400 font-medium">Adaptive Engine</span><span className="text-fuchsia-400 font-black">Active</span></div>
                </div>
             </motion.div>
          </div>
              ...
              {answerHistory.map((item, index) => (
                <motion.div variants={slideUpItem} key={index} className={`p-8 rounded-[2rem] border relative overflow-hidden backdrop-blur-md transition-all hover:scale-[1.01] ${item.isCorrect ? 'bg-emerald-950/20 border-emerald-900/50 shadow-[0_0_30px_rgba(16,185,129,0.05)]' : 'bg-red-950/20 border-red-900/50 shadow-[0_0_30px_rgba(244,63,94,0.05)]'}`}>
                  ...
                  <div className="flex items-start justify-between mb-6 pl-4">
                    <h3 className="text-xl font-bold text-white leading-relaxed pr-6 whitespace-pre-wrap"><span className="text-slate-500 mr-2">0{index + 1}.</span> {formatAIText(item.question.text)}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4">
                    <div className="bg-[#0B0F19]/60 p-5 rounded-2xl border border-slate-800">
                      ...
                      <span className={`text-lg font-bold whitespace-pre-wrap ${item.isCorrect ? "text-emerald-400" : "text-red-400"}`}>{item.selectedOption}: {formatAIText(item.question.options[item.selectedOption])}</span>
                    </div>
                    {!item.isCorrect && (
                      <div className="bg-emerald-950/30 p-5 rounded-2xl border border-emerald-900/50">
                        ...
                        <span className="text-emerald-400 text-lg font-bold whitespace-pre-wrap">{item.question.correct}: {formatAIText(item.question.options[item.question.correct])}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {!item.isCorrect && item.question.explanation && (
                    <div className="mt-6 ml-4 bg-[#0B0F19]/80 p-5 rounded-2xl border-l-4 border-fuchsia-500 flex items-start">
                      ...
                      <p className="text-slate-300 font-medium leading-relaxed whitespace-pre-wrap"><strong className="text-fuchsia-400 font-bold uppercase text-xs tracking-widest block mb-1">AI Insights</strong> {formatAIText(item.question.explanation)}</p>
                    </div>
                  )}
                </motion.div>
              ))}
